...
def train_model(num_classes, epochs, train_dataset, validation_dataset):
    ...
    checkpoint_filepath = '/home/rodrigocm/IQA-Retinal-Project/algorithms/modelv3/weights_inceptionv3/.'
...
# FUNCTION: plot and save results
def plot_metric_epochs(train_data, val_data, train_label, val_label, x_label, y_label, title, file_name):
    ...
    plt.legend(loc='best')
    plt.savefig("./graphsInception/" + file_name + ".png")
    plt.close()
...
def confusion_matrix_and_report(model, test_dataset):
    ...
    plt.savefig("./graphsInception/confusion_matrix.png")
...
def plotROCCurve(y_true, Y_pred):
    ...
    plt.title('ROC curve')
    plt.legend(loc='best')
    plt.savefig("./graphsInception/roc_curve.png") 
    plt.clf()
...
def plotPRCurve(y_true, Y_pred):
    ...
    plt.legend(loc='best')
    plt.savefig("./graphsInception/pr_curve.png")
    plt.show()
    plt.clf()
